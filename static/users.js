...
function send_update(element) {
    ...
    var username = parent.children[1].value;
    var email = parent.children[2].value;
    var is_admin = parent.children[3].checked;
    var verify_code = parent.children[4].value;
    var api_key = parent.children[5].value;
    var password = parent.children[6].value;
    // validate username
    ...
    var data = { "id": user_id, "username": username, "email": email, "is_admin": is_admin, "verify_code": verify_code, "api_key": api_key, "password": password }
    $.ajax({
        url: "/manage_user_backend",
        type: "patch",
